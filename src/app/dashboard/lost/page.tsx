...
export default function LostPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Lost</h1>
        <p className="text-slate-400 text-sm mt-0.5">
          Contacts and demos marked as Lost · {data?.total ?? 0} total
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 bg-slate-900/40 overflow-hidden">
          ...
          {(!data?.data?.length || data.data.length === 0) && (
            <p className="p-8 text-center text-slate-500">No lost records yet.</p>
          )}
        </div>
      )}
    </div>
